...
class DeeplinkSchemaMatch {
  static hasSchema(schemaString: string): boolean {
    ...
    return (
      ...
      lowercaseString.startsWith('kaiwallet:') ||
  ...
  static navigationRouteFor(
    ...
    event.url = event.url.replace(/^kaiwallet:\/\//i, 'bluewallet://');
    event.url = event.url.replace(/^kaiwallet:/i, 'bluewallet:');
  ...
  static getServerFromSetElectrumServerAction(url: string): string | false {
    const u = url.replace(/^kaiwallet:/i, 'bluewallet:');
    if (!u.startsWith('bluewallet:setelectrumserver') && !u.startsWith('setelectrumserver')) return false;
    const splt = u.split('server=');
  ...
  static getUrlFromSetLndhubUrlAction(url: string): string | false {
    const u = url.replace(/^kaiwallet:/i, 'bluewallet:');
    if (!u.startsWith('bluewallet:setlndhuburl') && !u.startsWith('setlndhuburl')) return false;
    const splt = u.split('url=');
    if (splt[1]) return decodeURIComponent(splt[1]);
    return false;
  }
